...
public abstract class AutoBaseTB3 extends BaseClassTB3 {
    ...
    int armSamplePos;
    double encoderAvg = 0;
    double lastEncoderFL = 0;
    double lastEncoderBR = 0;
    double dFCMPrev = 0;
    ...
    double targetDRIVE_TO_CRATER_Gold;
    double lowerSSTime;
    double stopTime;
    boolean backupDTW = false;
    ...
    //Keeps the arm at desired level
    public void armLevel(int level, int tolerance) {

        int mAPos = mA.getCurrentPosition();
        //int tol = tolerance / 2;

        if (mAPos < level) {
            mA.setPower(0.1);
        } else if (mAPos > level - tolerance && mAPos < level + tolerance) {
            mA.setPower(constantArm());
        } else {
            mA.setPower(-0.2);
        }
    }
    ...
    public void getEncoderAvg() {
        encoderAvg = ((mFL.getCurrentPosition() - lastEncoderFL) + (mBR.getCurrentPosition() - lastEncoderBR)) / 2;
    }
    ...
    public void resetSteps() {
        ...
        lastEncoderFL = mFL.getCurrentPosition();
        lastEncoderBR = mBR.getCurrentPosition();
        runtime.reset();
        stopDriveTrain();
    }
    ...
    public void updateMineralTarget() {
        armSamplePos = -50;
        targetDRIVE_TO_CRATER = 500;
        targetDRIVE_TO_CRATER_Gold = 400;
        ...
        if (goldPos == 1 || goldPos == 0) {
            ...
            targetSAMPLE = 2400;
            targetSAMPLE_Silver = 1000;
            targetDRIVE_TO_WALL = 2100;
