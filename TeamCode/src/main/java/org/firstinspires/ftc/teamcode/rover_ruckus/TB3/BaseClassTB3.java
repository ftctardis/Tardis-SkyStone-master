...
public abstract class BaseClassTB3 extends LinearOpMode {
    ...
    public double armSlowUp(){
        int currPosition = mA.getCurrentPosition();
        double armSlow = -((Math.pow(currPosition - 1500, 2)) / (434783)) + 4;
    ...
    public double armSlowDown() {
        int currPosition = mA.getCurrentPosition();
        double armSlow;

        if (currPosition > 1600) {
            armSlow = -((Math.pow(currPosition - 1600, 2)) / (30000)) + 6;

            if (armSlow < 3.5){
                armSlow = 3.5;
            }
        } else {
            armSlow = -((Math.pow(currPosition - 1600, 2)) / (230000)) + 6;
    ...
    public void defineComponents() {
        ...
        mBR.setMode(DcMotor.RunMode.RUN_USING_ENCODER); //TB3 ENCODERS FL BR
